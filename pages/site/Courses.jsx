...
function Courses() {
  ...
  async function getInfo() {
    const info = await getOrdersByEmail(user.email);
    const userCourses = info.filter((info) => info.userEmail == user.email);

    setCoursesToShow(userCourses);
  }
  return (
    <div>
      {coursesToShow.map((userCourse) => {
        const { courseId, courseName, details, demoID, _id, purchaseDate } =
          userCourse;
        const dateObj = {
          nanoseconds: purchaseDate.nanoseconds,
          seconds: purchaseDate.seconds,
        };
        const date = new Date(
          dateObj.seconds * 1000 + dateObj.nanoseconds / 1000000
        );
        const formattedDate = date.toLocaleDateString(); // Formato local de fecha en string
            ...
            >
              <div className="md:flex items-center justify-between w-full bg-gray-50">
                ...
                <div className="xl:w-4/6 md:w-1/2 w-full xl:pl-48">
                  ...
                  >
                    “Don’t get carried away with super-short paragraphs. The
                    secret to one-sentence paragraphs is to use them like a
                    spice: A pinch of cumin can make a meal more savory, but I
                    don’t want to eat a plateful of the stuff.”
                  </p>
                  <div className="w-full">
                    <VideoPlayer source={courseId} />
